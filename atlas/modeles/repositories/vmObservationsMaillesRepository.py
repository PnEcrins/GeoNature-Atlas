...
def getObservationsMaillesChilds(session, cd_ref, year_min=None, year_max=None):
    ...
    if year_min and year_max:
        query = query.filter(extract('year', VmObservations.dateobs).between(year_min, year_max))
...
def lastObservationsMailles(connection, mylimit, idPhoto):
    sql = """
        ...
        LEFT JOIN atlas.vm_medias medias
            ON medias.cd_ref = o.cd_ref AND medias.id_type = 1
...
FROM obs_in_area
         JOIN atlas.vm_observations obs ON obs.id_observation = obs_in_area.id_observation
         JOIN atlas.vm_l_areas vla ON vla.id_area=obs.id_area
